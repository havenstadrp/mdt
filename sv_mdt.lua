...
AddEventHandler("mdt:getOffensesAndOfficer", function()
	...
	local charges = {}
<<<<<<< HEAD
	exports.oxmysql:fetch('SELECT * FROM fine_types', {
	}, function(fines)
=======
	exports.oxmysql:fetch('SELECT * FROM fine_types', {}, function(fines)
>>>>>>> 9fe7d0d4c22bfd0713e3d3ace882a6fc4c053e70
		for j = 1, #fines do
...
AddEventHandler("mdt:performOffenderSearch", function(query)
	...
	local matches = {}
<<<<<<< HEAD
	exports.oxmysql:fetch("SELECT * FROM `players` WHERE `charinfo` LIKE @query", {
...
<<<<<<< HEAD
    exports.oxmysql:fetch('SELECT * FROM `user_mdt` WHERE `char_id` = @id', {
		['@id'] = offender.id
	}, function(result)
=======
...
<<<<<<< HEAD
        exports.oxmysql:fetch('SELECT * FROM `user_convictions` WHERE `char_id` = @id', {
            ['@id'] = offender.id
...
<<<<<<< HEAD
            exports.oxmysql:fetch('SELECT * FROM `mdt_warrants` WHERE `char_id` = @id', {
                ['@id'] = offender.id
            }, function(warrants)
=======
...
<<<<<<< HEAD
				exports.oxmysql:fetch('SELECT * FROM `player_vehicles` WHERE `citizenid` = @citizenid', {
...
AddEventHandler("mdt:getOffenderDetailsById", function(char_id)
    local usource = source
<<<<<<< HEAD
    exports.oxmysql:fetch('SELECT * FROM `players` WHERE `id` = @id', {
		['@id'] = char_id
	}, function(result)
=======
...
<<<<<<< HEAD
	exports.oxmysql:fetch('SELECT * FROM `user_mdt` WHERE `char_id` = @id', {
		...
		if result[1] then
			exports.oxmysql:execute('UPDATE `user_mdt` SET `notes` = @notes, `mugshot_url` = @mugshot_url, `bail` = @bail WHERE `char_id` = @id', {
				['@id'] = id,
				['@notes'] = changes.notes,
			...
			exports.oxmysql:execute('INSERT INTO `user_mdt` (`char_id`, `notes`, `mugshot_url`, `bail`) VALUES (@id, @notes, @mugshot_url, @bail)', {
				['@id'] = id,
				['@notes'] = changes.notes,
				['@mugshot_url'] = changes.mugshot_url,
				['@bail'] = changes.bail
		...
		if changes.convictions ~= nil then
			for conviction, amount in pairs(changes.convictions) do	
				exports.oxmysql:execute('UPDATE `user_convictions` SET `count` = @count WHERE `char_id` = @id AND `offense` = @offense', {
					['@id'] = id,
					['@count'] = amount,
					['@offense'] = conviction
				})
...
<<<<<<< HEAD
			exports.oxmysql:execute('DELETE FROM `user_convictions` WHERE `char_id` = @id AND `offense` = @offense', {
				['@id'] = id,
...
<<<<<<< HEAD
	exports.oxmysql:execute('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`) VALUES (@id, @title, @incident, @charges, @author, @name, @date)', {
		['@id']  = data.char_id,
		['@title'] = data.title,
		['@incident'] = data.incident,
		['@charges'] = charges,
...
<<<<<<< HEAD
		exports.oxmysql:fetch('SELECT * FROM `user_convictions` WHERE `offense` = @offense AND `char_id` = @id', {
			['@offense'] = offense,
			['@id'] = data.char_id
		}, function(result)
			if result[1] then
				exports.oxmysql:execute('UPDATE `user_convictions` SET `count` = @count WHERE `offense` = @offense AND `char_id` = @id', {
					['@id']  = data.char_id,
					['@offense'] = offense,
					['@count'] = count + 1
				})
			else
				exports.oxmysql:execute('INSERT INTO `user_convictions` (`char_id`, `offense`, `count`) VALUES (@id, @offense, @count)', {
					['@id']  = data.char_id,
					['@offense'] = offense,
					['@count'] = count
				})
...
AddEventHandler("mdt:performVehicleSearch", function(query)
	...
	local matches = {}
<<<<<<< HEAD
	exports.oxmysql:fetch("SELECT * FROM `player_vehicles` WHERE LOWER(`plate`) LIKE @query", {
		['@query'] = string.lower('%'..query..'%') -- % wildcard, needed to search for all alike results
	}, function(result)
=======
...
<<<<<<< HEAD
    			exports.oxmysql:fetch("SELECT * FROM `player_vehicles` WHERE `plate` = @query", {
					['@query'] = query
				}, function(result)
=======
...
AddEventHandler("mdt:getVehicle", function(vehicle)
	local usource = source
<<<<<<< HEAD
    exports.oxmysql:fetch("SELECT * FROM `players` WHERE `citizenid` = @query", {
...
<<<<<<< HEAD
        exports.oxmysql:fetch('SELECT * FROM `vehicle_mdt` WHERE `plate` = @plate', {
            ['@plate'] = vehicle.plate
        }, function(data)
=======
...
<<<<<<< HEAD
                exports.oxmysql:fetch('SELECT `bail` FROM user_mdt WHERE `char_id` = @id', {
                    ['@id'] = vehicle.owner_id
                }, function(bail)
=======
...
AddEventHandler("mdt:submitNewWarrant", function(data)
	...
	data.date = os.date('%m-%d-%Y %H:%M:%S', os.time())
<<<<<<< HEAD
	exports.oxmysql:execute('INSERT INTO `mdt_warrants` (`name`, `char_id`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (@name, @char_id, @report_id, @report_title, @charges, @date, @expire, @notes, @author)', {
		['@name']  = data.name,
...
AddEventHandler("mdt:getReportDetailsById", function(query, _source)
	...
	local usource = source
<<<<<<< HEAD
	exports.oxmysql:fetch("SELECT * FROM `mdt_reports` WHERE `id` = @query", {
		['@query'] = query
	}, function(result)
=======
...
<<<<<<< HEAD
	exports.oxmysql:fetch('SELECT * FROM `vehicle_mdt` WHERE `plate` = @plate', {
		...
		if result[1] then
			exports.oxmysql:execute('UPDATE `vehicle_mdt` SET `stolen` = @stolen, `notes` = @notes WHERE `plate` = @plate', {
				['@plate'] = data.plate,
				['@stolen'] = data.stolen,
				['@notes'] = data.notes
			})
		else
			exports.oxmysql:execute('INSERT INTO `vehicle_mdt` (`plate`, `stolen`, `notes`) VALUES (@plate, @stolen, @notes)', {
				['@plate'] = data.plate,
				['@stolen'] = data.stolen,
				['@notes'] = data.notes
			})
